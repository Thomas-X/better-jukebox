...
public class ItemHandlerJukeBox implements IItemHandlerModifiable {
    ...
    public ItemStack extractItem(int slot, int amount, boolean simulate) {
        ...
        if (!simulate) {
            IJukeboxBlockTileEntityMixin jukeboxBlockTileEntityMixin = (IJukeboxBlockTileEntityMixin) jukebox;

            if (jukeboxBlockTileEntityMixin.getIsPlaying()) {
                return ItemStack.EMPTY;
            }

            setPlayState(jukebox, false);

            jukeboxBlockTileEntityMixin.reset();
